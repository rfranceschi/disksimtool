...
try:
    ...
    n_a = len(opac_dict['a'])
    for i, _lam in enumerate(lam_obs_list):
        ilam = np.nonzero(lam_opac < _lam)[0][-1]
        if not lam_obs_list[i] == lam_opac[ilam]:
            logging.info(
...
for _lam_image in lam_obs_list:
    ...
    try:
        ...
        # radmc_out_path.replace(radmcfolder / 'image.out')
        im_sim.writeFits(str(fits_path), dpc=options['distance'], coord='11h01m51.9053285064s -34d42m17.033218380s')
    except FileNotFoundError:
